...
def gcn_norm(edge_index, edge_weight=None, num_nodes=None,
             add_self_loops=False, dtype=None, alpha0=-0.5, alpha1=-0.5):
...
class Diffuse(MessagePassing):
    ...
    def __init__(
        self, in_channels, out_channels, trainable_laziness=False, fixed_weights=True, alpha0=-0.5, alpha1=-0.5
    ):
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        edge_index, edge_weight = gcn_norm(edge_index, edge_weight, x.size(self.node_dim), dtype=x.dtype, alpha0=self.alpha0, alpha1=self.alpha1)
...
class Scatter(torch.nn.Module):
    def __init__(self, in_channels, trainable_laziness=False, agg='moment', alpha=0):
        ...
        alpha0 = -0.5 + alpha
        alpha1 = -0.5 - alpha
        self.diffusion_layer1 = Diffuse(in_channels, in_channels, trainable_laziness, alpha0=alpha0, alpha1=alpha1)
        self.diffusion_layer2 = Diffuse(
            4 * in_channels, 4 * in_channels, trainable_laziness, alpha0=alpha0, alpha1=alpha1
        )
